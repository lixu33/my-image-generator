...
export default function () {
  ...
  const fetchImages = async (page: number) => {
    try {
      ...
      console.log("fetch images", code, message, data);
      if (data) {
        setImages(data);
      }
    } catch (e) {
    ...
    <section>
      <div className="mx-auto max-w-7xl px-5 my-16">
        <div className="mx-auto w-full max-w-3xl text-center">
          <h2 className="text-3xl font-normal md:text-2xl ">全部图片</h2>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8 md:grid-cols-3 lg:gap-12">
          {loading ? (
            ...
            <>
              {images &&
                images.map((image: MyImage, idx: number) => {
                  return (
                    <div
                      key={idx}
